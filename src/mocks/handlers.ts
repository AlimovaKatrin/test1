...
export const handlers = [
  http.get('/api/user', async () => {
    ...
    if (mode === 'user-500' || mode === 'partial-failure')
      return HttpResponse.json({ message: 'User unavailable' }, { status: 500 });
    if (mode === 'network-error') return HttpResponse.error();
    if (mode === 'malformed-payload' || mode === 'malformed-json')
  ...
  http.get('/api/transactions', async () => {
    ...
    if (mode === 'transactions-500')
      return HttpResponse.json({ message: 'Transactions unavailable' }, { status: 500 });
    if (mode === 'network-error') return HttpResponse.error();
    if (mode === 'empty-transactions') return HttpResponse.json([]);
    if (mode === 'invalid-amount')
      return HttpResponse.json([{ ...transactions[0], amount: 'not-money' }]);
