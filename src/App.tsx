...
function mapTransactions(value: unknown): Transaction[] {
  ...
  return value.map((item) => {
    if (
      ...
      !['COMPLETED', 'PENDING', 'FAILED'].includes(String(item.state))
    )
    ...
    const dto: TransactionDto = {
      ...
      state: item.state as TransactionDto['state'],
    };
    return { ...dto, amount };
  });
}
...
function App() {
  ...
  const loadUser = useCallback(async () => {
    ...
    try {
      const response = await checkedResponse('/api/user');
      setUser({ status: 'success', data: mapUser(await readJson(response)) });
    } catch (error: unknown) {
      setUser({ status: 'error', error: toError(error) });
    }
  }, []);
  ...
  const loadTransactions = useCallback(async () => {
    ...
    try {
      const response = await checkedResponse('/api/transactions');
      setTransactions({ status: 'success', data: mapTransactions(await readJson(response)) });
    } catch (error: unknown) {
      setTransactions({ status: 'error', error: toError(error) });
    }
  }, []);
  ...
  return (
    <main>
      ...
      <div className="grid">
        <section className="card" aria-labelledby="user-heading">
          <h2 id="user-heading">Current user</h2>
          {user.status === 'loading' || user.status === 'idle' ? (
            <p role="status">Loading user…</p>
          ) : null}
          {user.status === 'error' ? (
            <div role="alert">
              <p className="error">Could not load user: {user.error.message}</p>
              <button onClick={() => void loadUser()}>Retry user</button>
            </div>
          ) : null}
          {user.status === 'success' ? (
            <p>
              ...
              <br />
              <span className="muted">Primary currency: {user.data.currency}</span>
            </p>
          ) : null}
        </section>
        <section className="card" aria-labelledby="transactions-heading">
          <h2 id="transactions-heading">Recent transactions</h2>
          {transactions.status === 'loading' || transactions.status === 'idle' ? (
            <p role="status">Loading transactions…</p>
          ) : null}
          {transactions.status === 'error' ? (
            <div role="alert">
              <p className="error">Could not load transactions: {transactions.error.message}</p>
              <button onClick={() => void loadTransactions()}>Retry transactions</button>
            </div>
          ) : null}
              ...
              {transactions.data.map((transaction) => (
                <li key={transaction.id}>
                  ...
                  <span className="muted">
                    {date(transaction.createdDate)} · {transaction.state}
                  </span>
                </li>
              ))}
            </ul>
